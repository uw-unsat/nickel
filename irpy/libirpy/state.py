...
class StructMeta(type):
    def __new__(cls, name, parents, dct):
        ...
        for k, v in sorted(list(dct.items()), key=lambda v: getattr(v[1], '_order', float('inf'))):
            if hasattr(v, '_init'):
                meta[k] = v
            if type(type(v)) == cls:
                meta[k] = v.__class__
                del dct[k]
        dct['_meta'] = collections.OrderedDict()
        for parent in parents:
            if hasattr(parent, '_meta'):
                dct['_meta'].update(parent._meta)

        dct['_meta'].update(meta)

        return super(StructMeta, cls).__new__(cls, name, parents, dct)


class Struct(with_metaclass(StructMeta, object)):
    __frozen = False
    ...
    def _init(self, parent, name):
        ...
        for k, v in list(self._structs.items()):
            v = copy.copy(v)
            self._structs[k] = v
            v._init(self, k)
        self.__frozen = True
    ...
    def equiv(self, other):
        ...
        for k in list(self._meta.keys()):
            if k in self._structs:
                conj.append(self._structs[k] == other._structs[k])
            else:
                types = [z3.Const('arg', i) for i in self._meta[k].get_types()[:-1]]
                k1 = getattr(self, k)
                k2 = getattr(other, k)
                conj.append(z3.ForAll(types, k1(*types) == k2(*types)))
        return z3.And(*conj)
    ...
    def initial(self):
        kp = self.copy()
        for k, v in list(kp._meta.items()):
            if k in kp._structs:
...
class Map(AbstractMap):
    def new(self):
        if self.get_arity() == 0:
            ...
            if typ == z3.BoolSort():
                ...
                value = util.FreshInt(self._prefix + self._name)
            else:
    ...
    def set(self, old, args, value):
        # Cast to a z3 bitvec
        if not z3.is_expr(value):
            if isinstance(value, int):
                value = self.get_types()[-1].cast(value)

        # "raw" assignment of a lambda without any accessor arguments
        if isinstance(value, types.FunctionType):
            def newfn(*innerargs):
                assert len(innerargs) >= len(args)
                cond = z3.And(*[a == b for a, b in zip(args, innerargs[:len(args)])])
                if args:
                    oldf = functools.partial(old, *args)
                else:
                    oldf = old
                return util.If(cond, value(*innerargs[len(args):], oldfn=oldf), old(*innerargs))
            return newfn
...
class Refcnt(Map):
    ...
    def set(self, old, args, value):
        ...
        assert all([isinstance(v, types.FunctionType) for v in value])
        return value
