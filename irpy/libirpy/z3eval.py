...
class BaseZ3IRPyEval(BaseIRPyEvaluator):
    ...
    def get_poison(self, type):
        ...
        raise Exception("Can't get posion for {!r}".format(type))
...
class Z3IRPyEval(BaseZ3IRPyEval):
    ...
    def get_element_ptr(self, ctx, return_type, target, target_type, *args, **kwargs):
        ...
        for i in args[::2]:
            if i.size() != 64:
                aargs.append(z3.ZeroExt(64 - i.size(), i))
            else:
                aargs.append(i)

        return target.getelementptr(ctx, *aargs, type=return_type)
    ...
    def ptr_to_int(self, ctx, return_type, value, value_type, **kwargs):
        ...
        assert len(path) <= 2 or all(map(lambda x: x.as_long() == 0, path))
    ...
    def int_to_ptr(self, ctx, *args, **kwargs):
        self.assertion(ctx, False, msg='int_to_ptr')
    inttoptr = int_to_ptr

    def bitcast(self, ctx, return_type, value, value_type, **kwargs):
        assert return_type.is_pointer()
        assert value_type.is_pointer()
        assert isinstance(value, dt.Pointer) or isinstance(value, dt.BitcastPointer)
        return value.bitcast(ctx, return_type)
    ...
    def declare_global_variable(self, ctx, name, global_type, value=None, value_type=None):
        def unknown_function(*args, **kwargs):
            raise Exception("Calling unknown function {}").format(name)
    ...
    def asm(self, ctx, inline_assembly):
        self.assertion(ctx, False, msg=repr(inline_assembly))
