...
static std::string escape_asm(const std::string &s)
{
  ...
  public:
    ...
    std::vector<std::pair<int, std::string>> getMetadata(void)
    {
        ...
        for (const auto &md : this->meta) {
            ...
            auto ident_string = line.substr(1, delim);
            if (!std::regex_match(ident_string, std::regex{"\\d+\\s*"}))
                continue;
            auto identifier = std::stoi(ident_string);
...
class PyInstVisitor : public llvm::InstVisitor<PyInstVisitor>
{
  ...
  public:
    ...
    template <typename T> void genPyCallFromInstruction(HasReturn hasReturn, const std::string &fname, T &i, kwargs_t kwargs = {})
    {
        args_t args;
    ...
    std::string get(const llvm::Value *val)
    {
        if (const llvm::Instruction *i = llvm::dyn_cast<llvm::Instruction>(val)) {
            ...
            return genPyCall("asm", {quote(escape_asm(ia->getAsmString()))});
        }
    ...
    void visitTruncInst(const llvm::TruncInst &i)
    {
        ...
        genPyCallFromInstruction(HasReturn::Yes, "trunc", i);
    }
    ...
    void visitZExtInst(const llvm::ZExtInst &i)
    {
        ...
        dst = get(&i);
        operand = get(i.getOperand(0));
        genPyCallFromInstruction(HasReturn::Yes, "zext", i);
    }
    ...
    void visitSExtInst(const llvm::SExtInst &i)
    {
        ...
        dst = get(&i);
        operand = get(i.getOperand(0));
        genPyCallFromInstruction(HasReturn::Yes, "sext", i);
    }

    void visitLoadInst(const llvm::LoadInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "load", i);
    }

    void visitStoreInst(const llvm::StoreInst &i)
    {
        genPyCallFromInstruction(HasReturn::No, "store", i);
    }

    void visitAllocaInst(const llvm::AllocaInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "alloca", i);
    }

    void visitPtrToIntInst(const llvm::PtrToIntInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "ptr_to_int", i);
    }

    void visitIntToPtrInst(const llvm::IntToPtrInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "int_to_ptr", i);
    }

    void visitBitCastInst(const llvm::BitCastInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "bitcast", i);
    }
    ...
    void visitGetElementPtrInst(const llvm::GetElementPtrInst &i)
    {
        ...
        genPyCallFromInstruction(HasReturn::Yes, "get_element_ptr", i, kwargs);
    }
    ...
    {
        HasReturn has_return = static_cast<HasReturn>(i.getType()->getTypeID() != llvm::Type::TypeID::VoidTyID);
        genPyCallFromInstruction(has_return, "call", i);
    }

    void visitUnreachableInst(const llvm::UnreachableInst &i)
    {
        genPyCallFromInstruction(HasReturn::No, "unreachable", i);
    }

    void visitInsertValueInst(const llvm::InsertValueInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "insertvalue", i);
    }

    void visitExtractValueInst(const llvm::ExtractValueInst &i)
    {
        genPyCallFromInstruction(HasReturn::Yes, "extractvalue", i);
    }
