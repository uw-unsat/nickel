...
def is_proc_valid(pid):
    return z3.And(z3.UGT(pid, 0), z3.ULT(pid, NR_PROCS))


def is_sched_idx_valid(idx):
    return z3.ULT(idx, NR_PROCS)


def parent(pid):
    return util.If(pid == 1, pid, z3.UDiv(pid - 2, NR_CHILDREN) + 1)


def child_pid(pid, child):
    return (pid - 1) * NR_CHILDREN + child + 2
...
def is_free_child(state, pid, child):
    return z3.And(
        z3.UGE(child, child_pid(pid, state.procs[pid].nr_children)),
        z3.ULE(child, child_pid(pid, NR_CHILDREN - 1)))
