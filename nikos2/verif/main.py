...
class MCertiKOSNI(ni.ProofBase):
    doms = [spec.ni.ProcDomain(util.FreshBitVec('dom_pid', dt.pid_t))]
...
class DomainValid(ni.DomainValid, MCertiKOSNI):
    prototypes = p.all_ops


class EqvTest(ni.EquivalenceProp):
    doms = [spec.ni.ProcDomain(util.FreshBitVec('dom_pid', dt.pid_t))]
    state = state.MCertiKOSState
...
class MCertiKOSSpecLemma(ni.ProofBase):
    ...
    def _test_handler(self, syscall, lemma):
        ...
        post = lemma(state)
        m = self._prove(post, pre=pre, return_model=self.INTERACTIVE)
        if m:
            from ipdb import set_trace; set_trace()
