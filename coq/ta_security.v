...
Section Noninterference.
  ...
  Section Intransitive.
    ...
    Definition ta (u: D) (a: list A) := ta_reverse u (rev a).

    Definition ta_security : Prop :=
      forall a alpha beta,
        ta (dom a) alpha = ta (dom a) beta ->
        output (run init alpha) a = output (run init beta) a.
    ...
    Section Unwinding.
      ...
      Lemma Empty_no_affect :
        forall alpha a,
          Empty = ta_reverse (dom a) (rev alpha) ->
          output init a = output (run init alpha) a.
      Proof.
        induction alpha using (rev_ind); intros; auto.
        rewrite do_run.
        apply output_consistency; auto.
        apply eqv_trans with (y := run init alpha).
        - admit.
        - unfold Unwinding.local_respect in local_respect; apply local_respect; auto.
          rewrite rev_unit in H.
          simpl in H.
          destruct (interferes (dom x) (dom a)); try congruence.
      Admitted.

      Theorem noninterference_unwinding : ta_security.
      Proof.
        unfold ta_security.
        intros a alpha.
        induction alpha using (rev_ind).
        - simpl.
          unfold ta; simpl.
          intros.
          apply Empty_no_affect; auto.
        - intros.
          unfold ta in H.
          rewrite rev_unit in H.
          simpl in H.
          destruct (interferes (dom x) (dom a)) eqn:?.
          + rewrite do_run.
            admit.
          + rewrite do_run.
            rewrite <- IHalpha.
            * apply output_consistency.
              ...
              apply safe_run.
              apply safe_init.
              unfold Unwinding.local_respect in local_respect.
              apply eqv_sym.
              apply local_respect.
              apply safe_run; apply safe_init; auto.
              auto.
            * unfold ta; auto.
      Qed.

      (* The following lemma combines output consistency & local respect. *)
      (* It is easier to understand as it doesn't use equivalence. *)

      Lemma local_consistency :
        forall s a b,
          safe s ->
          interferes (dom a s) (dom b s) = false ->
          output s b = output (step s a) b.
      Proof.
        auto.
      Qed.
  ...
  Lemma ipurge_ipurge_r :
    forall M dom dom_r,
      ...
      forall alpha alpha' (sr sr': SR) a,
        ipurge M dom alpha (dom a (projT1 sr)) (projT1 sr') alpha' ->
        ipurge_r M dom_r alpha (dom_r a sr) sr' alpha'.
  Proof.
...
Section TNL.
  ...
  Lemma sources_no_contains :
    forall M alpha dom s u u',
      (forall u v, interferes u v = false) ->
      sources M dom alpha u s u' ->
      u = u'.
  Proof.
    induction alpha; intros.
    - inversion H0; auto.
  ...
  Proof.
    ...
    - unfold transitive_nonleakage.
      ...
      unfold eqv_list.
      intros.
      apply sources_no_contains in H3; subst; auto.
...
Section Refinement.
  ...
  Section ImplementationNoninterference.
    ...
    Proof.
      apply (noninterference_unwinding_r Impl impl_dom impl_eqv).
