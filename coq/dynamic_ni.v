...
Section Noninterference.
  Variable Spec : M.
  Definition policy : Type := S -> D -> D -> bool.
  ...
  Section Intransitive.
    ...
    Definition noninterference : Prop :=
      forall s s' alpha alpha' a,
        s = run init alpha ->
        s' = run init alpha' ->
        ipurge alpha (dom a s) init alpha' ->
    ...
    Section Unwinding.
      ...
      Definition dom_respect :=
        forall u s t a,
          ...
          safe t ->
          eqv u s t ->
          (interferes s (dom a s) u) =
          (interferes t (dom a t) u).
      ...
      Proof.
        ...
        intros.
        assert (interferes t (dom a t) u = false).
        erewrite <- H; eauto.
      ...
      Proof.
        ...
        destruct (interferes s (dom a s) v) eqn:?.
        ...
        - apply eqv_trans with (y := s).
          ...
          + assert (interferes t (dom a t) v = false).
            * rewrite <- Heqb.
      ...
      Proof.
        ...
        assert (~ interferes s (dom a s) u0 = true).
        contradict H.
        eapply sources_contains; eauto.
        destruct (interferes s (dom a s) u0) eqn:?; subst; intuition.
      Qed.
      ...
      Theorem noninterference_unwinding : noninterference.
      Proof.
        unfold noninterference.
        apply two.
        intros.
        apply five; eauto.
        unfold eqv_list; intros; apply eqv_refl.
      Qed.
      ...
      Proof.
        ...
        - apply IHalpha; auto.
          + intros.
            destruct (interferes s (dom a s) u0) eqn:?.
            * apply weak_step_consistency; auto.
              apply H1.
              now constructor.
              apply H1.
  ...
  Section Monotonicity.
    ...
    Definition restrictive (p q: policy) : Prop :=
      forall s u v, (p s u v = true) -> (q s u v = true).

    Lemma sources_monotonicity :
      forall interferes interferes' alpha u s v,
        restrictive interferes interferes' ->
        sources interferes alpha u s v ->
        sources interferes' alpha u s v.
   Proof.
     unfold restrictive.
     induction alpha; intros.
   ...
   Proof.
     ...
     - apply H0. eapply sources_monotonicity with (interferes := interferes); auto.
     -  apply H0.
   ...
   Lemma ipurge_monotonicity :
      forall interferes interferes' alpha alpha' u s,
        restrictive interferes interferes' ->
        ipurge interferes' alpha u s alpha' ->
        ipurge interferes alpha u s alpha'.
   Proof.
     induction alpha.
     intros.
...
Section ReachableNoninterference.
  Variable Impl : M.
  Definition policy_r : Type := SR -> D -> D -> Prop.
  Variable dom_r: A -> SR -> D.

  Section Intransitive.

    Variable interferes_r : SR -> D -> D -> bool.
    ...
    | sources_r_contains :
        forall alpha u s a v,
          ...
          interferes_r s (dom_r a s) v = true ->
    ...
    Definition noninterference_r : Prop :=
      forall s s' alpha alpha' a,
        ...
        ipurge_r alpha (dom_r a s) init_r alpha' ->
        output (projT1 s) a = output (projT1 s') a.
    ...
    Section Unwinding.
      ...
      Definition dom_respect_r :=
        forall u s t a,
          eqv_r u s t ->
          interferes_r s (dom_r a s) u = interferes_r t (dom_r a t) u.

      Definition output_consistency_r :=
        forall s t a,
          eqv_r (dom_r a s) s t ->
          output (projT1 s) a = output (projT1 t) a.

      Definition local_respect_r :=
        forall s a u,
          interferes_r s (dom_r a s) u = false ->
      ...
      Proof.
        ...
        destruct (interferes_r s (dom_r a s) v) eqn:?; subst.
        ...
        - apply eqv_r_trans with (y := s).
          ...
          + assert (interferes_r t (dom_r a t) v = false).
            * rewrite <- Heqb.
      ...
      Proof.
        ...
        assert (interferes_r s (dom_r a s) u0 <> true).
        contradict H.
        eapply sources_r_contains; eauto.
        destruct (interferes_r s (dom_r a s)) eqn:?; subst; auto.
        contradict H1; auto.
      Qed.
      ...
      Theorem noninterference_unwinding_r : noninterference_r.
      Proof.
        unfold noninterference_r.
        apply two_r.
        intros.
      ...
      Proof.
        ...
        - apply IHalpha; auto.
          intros.
          destruct (interferes_r s (dom_r a s) u0) eqn:?.
          + apply weak_step_consistency_r; auto.
            apply H.
            now constructor.
            apply H.
...
Section NI_NIR.
  ...
  Lemma sources_sources_r :
    forall M alpha dom dom_r interferes interferes_r,
      (forall (sr: SR) a,
          dom a (projT1 sr) = dom_r a sr) ->
      (forall (sr: SR) u v,
          interferes (projT1 sr) u v = interferes_r sr u v) ->
      forall sr u u',
        sources M dom interferes alpha u (projT1 sr) u' ->
        sources_r M dom_r interferes_r alpha u sr u'.
  Proof.
    intros M alpha dom dom_r interferes interferes_r dom_consistent interferes_consistent.
    induction alpha; intros.
    - inversion H. subst.
  ...
  Lemma ipurge_ipurge_r :
    forall M dom dom_r interferes interferes_r,
      (forall (sr: SR) a,
          dom a (projT1 sr) = dom_r a sr) ->
      (forall (sr: SR) u v,
          interferes (projT1 sr) u v = interferes_r sr u v) ->
      forall alpha alpha' (sr sr': SR) a,
        ipurge M dom interferes alpha (dom a (projT1 sr)) (projT1 sr') alpha' ->
        ipurge_r M dom_r interferes_r alpha (dom_r a sr) sr' alpha'.
  Proof.
    intros M dom dom_r interferes interferes_r dom_consistent interferes_consistent.
  ...
  Lemma noninterference_r_noninterference :
    forall M dom dom_r interferes interferes_r,
      ...
      (forall (sr: SR) u v,
          interferes (projT1 sr) u v = interferes_r sr u v) ->
      noninterference_r M dom_r interferes_r ->
      noninterference M dom interferes.
  Proof.
  ...
  Lemma nonleakage_r_nonleakage :
    forall M dom dom_r interferes interferes_r eqv eqv_r,
      (forall (sr: SR) a,
          dom a (projT1 sr) = dom_r a sr) ->
      (forall (sr: SR) u v,
          interferes (projT1 sr) u v = interferes_r sr u v) ->
      ...
      nonleakage_r M dom_r interferes_r eqv_r ->
      nonleakage M dom interferes eqv.
  Proof.
    unfold nonleakage_r, nonleakage.
    intros M dom dom_r interferes interferes_r eqv eqv_r dom_consistent interferes_consistent eqv_consistent.
    intuition.
    specialize H with (s := existT _ s H0)
                      (t := existT _ t H1)
                      (alpha := alpha)
                      (u := u).
    erewrite <- eqv_consistent in H.
    erewrite run_r_run in H.
    rewrite run_r_run in H.
...
Section TNL.
  ...
  Lemma sources_no_contains :
    forall M alpha dom s u u',
      sources M dom (fun _ _ _ => false) alpha u s u' ->
      u = u'.
  Proof.
    induction alpha; intros.
    - inversion H; auto.
  ...
  Lemma transitive_nonleakage_intransitive :
    forall M dom eqv,
      forall interferes,
        transitive_nonleakage M eqv ->
        nonleakage M dom interferes eqv.
  Proof.
    unfold nonleakage, transitive_nonleakage.
    intros.
    unfold eqv_list in H2.
    apply H; auto.
    unfold eqv_list in H2.
  ...
  Lemma nonleakage_transitive_nonleakage :
    forall M dom eqv,
        nonleakage M dom (fun _ _ _ => false) eqv <->
        transitive_nonleakage M eqv.
  Proof.
    intuition.
    - unfold transitive_nonleakage.
      ...
      unfold eqv_list.
      intros.
      apply sources_no_contains in H3.
      subst.
      auto.
    - apply transitive_nonleakage_intransitive; auto.
  Qed.
...
Section Refinement.
  ...
  Lemma bisimulation :
    forall alpha a,
    output (@do Impl alpha) a = output (@do Spec alpha) a.
  Proof.
    intros.
  ...
  Section ImplementationNoninterference.
    ...
    Variable spec_interferes : policy Spec.
    ...
    Definition impl_interferes (sr: SR) (u v: D) : bool :=
      spec_interferes (projT1 (abs sr)) u v.
    ...
    Hypothesis spec_dom_respect : dom_respect Spec spec_dom spec_interferes spec_eqv.
    Hypothesis spec_output_consistency : output_consistency Spec spec_dom spec_eqv.
    Hypothesis spec_local_respect : local_respect Spec spec_dom spec_interferes spec_eqv.
    ...
    Hypothesis spec_step_respect : step_respect Spec spec_dom spec_interferes spec_eqv.
    ...
    Lemma impl_dom_respect :
      dom_respect_r Impl impl_dom impl_interferes impl_eqv.
    Proof.
    ...
    Lemma impl_output_consistency :
      output_consistency_r Impl impl_dom impl_eqv.
    Proof.
    ...
    Lemma impl_local_respect :
      local_respect_r Impl impl_dom impl_interferes impl_eqv.
    Proof.
    ...
    Lemma impl_step_respect :
      step_respect_r Impl impl_dom impl_interferes impl_eqv.
    Proof.
    ...
    Lemma impl_noninterference_r :
      noninterference_r Impl impl_dom impl_interferes.
    Proof.
      apply (noninterference_unwinding_r Impl impl_dom impl_interferes impl_eqv).
    ...
    Lemma impl_nonleakage_r :
      nonleakage_r Impl impl_dom impl_interferes impl_eqv.
    Proof.
      apply (nonleakage_unwinding_r Impl impl_dom impl_interferes impl_eqv).
      - apply impl_weak_step_consistency.
    ...
    Theorem impl_nonleakage:
      forall dom interferes eqv,
        (forall ir a,
            dom a (projT1 ir) = impl_dom a ir) ->
        (forall ir u v,
            interferes (projT1 ir) u v = impl_interferes ir u v) ->
        (forall d ir jr,
            eqv d (projT1 ir) (projT1 jr) = impl_eqv d ir jr) ->
        nonleakage Impl dom interferes eqv.
    Proof.
      intros.
    ...
    Theorem impl_noninterference :
      forall dom interferes,
        (forall ir a,
            dom a (projT1 ir) = impl_dom a ir) ->
        (forall ir u v,
            interferes (projT1 ir) u v = impl_interferes ir u v) ->
        noninterference Impl dom interferes.
    Proof.
      intros.
